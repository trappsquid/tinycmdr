"""`/cmdr <verb>`: the one command trigger that always gets through.

Why this exists (operator, 2026-09-22: "lets use /cmdr to invoke things within
tinycmdr"): a chat client never sends a message that starts with "/" unless it
...
only arrives because the relay has a row for it. `/cmdr` is one registered trigger
that carries anything, and the build has to accept the prefix in every lane it
serves (chat, web page, Telegram DM, console) while the bare words keep working:
the relay's per-verb rows post those.

What must hold, and what this pins:
  * the mapping, including the shapes that must NOT be treated as a prefix
    (`/cmdrmodel`, `/cmdrfoo`, a task that merely mentions /cmdr)
  * the chat handler: `/cmdr new`, `/cmdr help`, an unknown verb answered locally
    (nothing queued, no model call)
  * the LISTENER thread: `/cmdr stop` and `/cmdr restart force` are read while a run
...
def test_shapes_that_are_not_a_prefix():
    for given in ("/cmdrmodel", "/cmdrfoo x", "/cmd", "", "   ",
                  "tell me about /cmdr", "run cmdr strip",
                  "/model list", "/new"):
        got = fb.cmdr_strip(given)
...
def test_chat_handler_routes_a_prefixed_command():
    ...
    d._handle("c1", "david", "/cmdr new", "m1", "m1", True)
    check("/cmdr new clears the session locally",
          any("Session cleared" in p for p in posted), posted)
    check("/cmdr new did not queue a task", not d.queues.get("c1"), d.queues)

    d2, posted2 = make_dispatcher()
    d2._handle("c1", "david", "/cmdr help", "m2", "m2", True)
    text = " ".join(posted2)
    check("/cmdr help answers with the command list",
          "**Commands:**" in text and "/cmdr <command>" in text, text[:200])
    check("/cmdr help queued nothing", not d2.queues.get("c1"), d2.queues)

    d3, posted3 = make_dispatcher()
    d3._handle("c1", "david", "/cmdr wibble", "m3", "m3", True)
...
def test_listener_thread_reads_a_prefixed_command():
    ...
    try:
        ...
        d.enqueue(FakeMessage("/cmdr stop", "m1"), "/cmdr stop")
        check("/cmdr stop lands with a run in flight and flags it",
              ev.is_set() and any("Stopping now" in p for p in posted), posted)
        check("/cmdr stop did not queue behind the run",
        ...
        d2.enqueue(FakeMessage("/cmdr restart force", "m2"), "/cmdr restart force")
        check("/cmdr restart force kills the live run then restarts",
              ev2.is_set() and len(calls) == 1 and not d2.queues.get("c1"),
              f"calls={calls} queues={d2.queues}")
        check("/cmdr restart force never answers 'queued'",
...
def test_web_lane():
    ...
    check("/cmdr status is a local reply, not a task", kind == "reply", kind)
    kind, reply = fb._web_command("/cmdr help", "web")
    check("/cmdr help lists the page's commands",
          kind == "reply" and "/cmdr <cmd>" in reply, reply[:160])
    kind, reply = fb._web_command("/cmdr wibble", "web")
    ...
    check("the page's command list advertises the prefix",
          any(c.startswith("/cmdr") for c, _h in fb.WEB_COMMANDS), fb.WEB_COMMANDS)


def test_console_lane():
    out = io.StringIO()
    with contextlib.redirect_stdout(out):
        keep = fb._cli_command("/cmdr help")
    printed = out.getvalue()
    check("/cmdr help in the console prints the command list",
          keep is True and "/help" in printed, printed[:120])

    out = io.StringIO()
    with contextlib.redirect_stdout(out):
        fb._cli_command("/cmdr wibble")
    ...
    check("the console help text documents the prefix",
          "/cmdr <CMD>" in fb.HELP_TEXT, fb.HELP_TEXT[:80])

    out = io.StringIO()
    with contextlib.redirect_stdout(out):
        fb._cli_while_running("/cmdr wibble")
