...
DEFAULT_CONFIG = {
    ...
    "mattermost": {
        ...
        "allowed_users": ["your-mattermost-username"],
        "ssl_verify": True,
    },
    "search": {
...
_DEFAULT_CORE = ("shell", "execute_code", "read_file", "write_file", "edit_file",
                 "skill", "task", "remember", "web_search", "fetch_url",
                 "list_tools", "find_tools", "ask_user")
...
- Work out every question you have about a big file FIRST and ask them in ONE call, instead of reading the same file again per question. If you do read one twice, the harness attaches its symbol map (every class and def with its line number) to the result: go straight to the region you need with read_file offset/limit.
- Narrate as you go: the operator watches the chat. Before each batch of tool calls, write ONE short plain-text line saying what you are about to check or do ("Checking what holds the file lock:", "Reading the last 50 lines of the service log:"). Under 15 words, no headers, no preamble — it is posted as its own message the moment you emit it, then the tools run.
- Prefer the OS's native mechanisms for routine maintenance — they are faster and safer than manual alternatives. On Windows: Windows Update (Microsoft.Update.Session COM or PSWindowsUpdate module), pnputil, winget, DISM, Get-ComputerInfo. On Linux: the system package manager, systemctl, journalctl, docker. Downloading installers from vendor websites or scraping download pages is the LAST resort for when native channels genuinely lack the software.
- Don't gold-plate. When the OS update channel offers a stable update, take it — chasing the vendor's absolute-latest version number is not the goal. Working and done beats perfect and pending.
...
def run_cli(once=None):
    ...
    def cli_ask_door(label="the console"):
        ...
        def opener(q, opts, wait, lbl):
            tail = ("\n  options: " + listy()) if opts else ""
            print(f"\n  ❓ {q}{tail}\n  answer (a number, or your own words; "
                  f"Enter to let the agent decide, {int(wait)}s max): ",
                  end="", flush=True)
            ...
            if a == "?" and opts:
                ...
                try:
                    ...
                    a = ""
            if a:
                row["answer"] = _ask_record_choice(row, a) or a
            row["ev"].set()
            return row
...
def validate_startup_config():
    ...
    if CONFIG["mattermost"].get("allowed_users") == ["your-mattermost-username"]:
        return ("mattermost.allowed_users still has the placeholder username.\n"
                "Set it to YOUR Mattermost username — the bot is "
                "deny-by-default, so nobody (including you) can use it "
                "until this is right.")
    return None
